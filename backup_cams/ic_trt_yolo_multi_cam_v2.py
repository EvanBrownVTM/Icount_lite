...
logging.basicConfig(filename='/home/joeyw/Desktop/logs/Icount.log', level=logging.DEBUG, format="%(asctime)-8s %(levelname)-8s %(message)s")
...
def infer_engine(timestr, frame0, frame2, frame_cnt0, frame_cnt2, cv_activities_cam0, cv_activities_cam2, cv_pick_cam0, cv_ret_cam0, cv_pick_cam2, cv_ret_cam2):
...
def process_actions(cv_act_cam0, cv_act_cam2, timeout = 3):
	...
	if door_state == "DoorOpened":
		...
		if cameras.IsGrabbing():
			...
			if all(check_list):
				...
				det_frame0, det_frame2, cart = infer_engine(timestr, frame0, frame2, frame_cnt0, frame_cnt2, cv_activities_cam0, cv_activities_cam2, cv_pick_cam0, cv_ret_cam0, cv_pick_cam2, cv_ret_cam2)
				
				act_picks, cv_pick_cam0, cv_pick_cam2 = process_actions(cv_pick_cam0, cv_pick_cam2, timeout = 3)
				act_returns, cv_ret_cam0, cv_ret_cam2 = process_actions(cv_ret_cam0, cv_ret_cam2, timeout = 3)
				
				
				
				if len(act_picks) > 0:
					#print(act_picks)
					for act_pick in act_picks:
						print("{} {} @ {}".format(act_pick[1], act_pick[0], act_pick[2]))
						cv_activities.append({'class_id': act_pick[0], 'action': act_pick[1], 'timestamp': act_pick[2]})
				if len(act_returns) > 0:
					#print(act_returns)
					for act_return in act_returns:
						print("{} {} @ {}".format(act_return[1], act_return[0], act_return[2]))
						cv_activities.append({'class_id': act_return[0], 'action': act_return[1], 'timestamp': act_return[2]})
