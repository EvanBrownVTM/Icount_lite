...
logging.basicConfig(filename='/home/joeyw/Desktop/logs/Post.log', level=logging.DEBUG, format="%(asctime)-8s %(levelname)-8s %(message)s")
...
def gen_video(transid):
	...
	for i in range(l):
		...
		cv2.putText(img_hstack, 'frame:' + str(i), (320, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
...
def gen_trans_summary(transid, cv_activities, ls_activities):
	...
	for i in range(len(m0_pick)):
		...
		activities.append({"id":pick_acts[ls_ind]['id'],
				...
				"cv_product_id": name2id[cv_pick[cv_ind][0]],
				"cv_validation": "True",
				"is_validation": "True",
	...
	for i in range(len(m0_ret)):
		...
		activities.append({"id":ret_acts[ls_ind]['id'],
				...
				"cv_confidence_score": random.randint(90,99),
				"cv_product_id": name2id[cv_ret[cv_ind][0]],
				"cv_validation": "True",
				"is_validation": "True",
