...
l_mask = np.int32(l_mask * 640)

def init():
	print('\tLoading TensoRT model...')
	# build the class (index/name) dictionary from labelmap file
	trt_yolo = TrtYOLO("yolov4-tiny-416", (416, 416), 5, False, path_folder = 'yolo/')
...
cameras = initializeCamera([cfg.camera_map["cam0"], cfg.camera_map["cam0_2"]])
	...
	if cameras.IsGrabbing():
		...
		if grabResult.GrabSucceeded():
			if cameraContextValue == 0:
				...
				frame2 = np.uint8(cv2.resize(np.uint8(grabResult.Array), (archive_size, archive_size)))
