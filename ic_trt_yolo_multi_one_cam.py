...
def initializeCamera(serial_number_list):	
	...
	pfs_list = ['ic_out_front.pfs', 'ic_side.pfs', 'ic_side_cam2.pfs']
	...
	if door_state == "DoorOpened":
		...
		if cameras.IsGrabbing():
			try:
				...
				cameras.Close()
				cameras, dev_len = initializeCamera([cfg.camera_map["cam0"], cfg.camera_map["cam0_1"], cfg.camera_map["cam0_2"]])
				check_list = [ False for i in range(dev_len)]
				cameras.StartGrabbing(pylon.GrabStrategy_LatestImageOnly)
				continue
			...
			if grabResult.GrabSucceeded():
				if cameraContextValue == 0:
					...
					frame1 = cv2.resize(np.uint8(grabResult.Array), (archive_size, int(archive_size / 1280 * 960)))
