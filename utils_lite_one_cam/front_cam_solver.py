...
class Product:
	...
	def infer(self, cart, logger, cv_activities, idle):
		#print(self._post_inference_couter)
		if self._perform_inference:
			self._inference(cart, logger, cv_activities, idle)
			self._clear_state()
	...
	def _inference(self, cart, logger, cv_activities, idle):
...
class FrontCam:
	...
	def interact_tracks(self, logger, cv_activities, idle = False):
		for id_, prod in self._tracks.items():
			prod.infer(self.cart, logger, cv_activities, idle)
