...
class SideCam:
	...
	def interact_tracks(self, idle = False):
		keys_to_remove = []
		res_actions = []
		for key, product in self._tracks.items():
			...
			if product._delay > self._max_delay and (len(product.hist) > 9 or product._is_added):
				actions = self.perform_inference(key, product.class_id, type = "remove", idle = idle)
				if actions:
					res_actions.append(actions)
				keys_to_remove.append(key)
			elif product._delay > self._max_delay:
				actions = self.perform_inference(key, product.class_id, idle = idle)
				if actions:
					res_actions.append(actions)
				keys_to_remove.append(key)
			...
			if not product.is_added:
				if product.interact:
					actions = self.perform_inference(key, product.class_id, type = "add", idle = idle)
					if actions:
						res_actions.append(actions)
	...
	def perform_inference(self, obj_id, class_id, type = None, idle = False):
		...
		if action != 'NO ACTION' and not idle:
			print('{} - {} {}'.format(self._cam_id, action, int(class_id)))
			if action == 'PICK':
